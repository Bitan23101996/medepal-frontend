...
import { FormGroup, ControlContainer, FormBuilder, Validators, FormControl } from '@angular/forms';
...
export class CommonPrescriptionComponent implements OnInit {
  ...
  @Input() prescriptionFlag: boolean;
  ...
  oldContainedValue: any;
  initialFormValue: any;
  editForApp: boolean = false;
  ...
  constructor(private controlContainer: ControlContainer,
    ...
    private frb: FormBuilder) { }//end of constructor

  ngOnInit() {
    this.loadInitialData();
    this.loadAllMasterData();
    //app#2367
    // this.addressForm = this.frb.group({
  ...
  loadInitialData() {
    ...
    //new add for online consultation
    // this.appointmentForm.addControl('onlineConsultation',new FormControl(false)); //added for online consultation
    ...
    if (mobile.length > 12) {
      ...
      this._doctorService.getAssociateUserByEaddress(mobile).subscribe(resp => {
        ...
        if (resp.status == 2000) {
          ...
          // console.log(document.getElementsByClassName("custom-p-sidebar-user-list-modal"));
  ...
  convertTiming(event, label) {
    this.convertTimingOnChange.emit({ event: event, label: label });
  }//end of method
  ...
  populateAssociatedPatientDetailsByRefNo(userRefNo){

    this._doctorService.getAssociateUserByRefNo(userRefNo).subscribe(resp => {
      this.associateUser = resp.data;
      this.emitAssociateUser.emit(this.associateUser);
      let user = JSON.parse(localStorage.getItem('user'));
      this.appointmentForm.patchValue({
        'patientName': this.associateUser.name,
        'patientGender': this.associateUser.gender,
        'patientRefNo': userRefNo,
        'appointmentByRefNo': userRefNo,
        'isExistingInUser': true,
        'isSerial': false,
        'forMinor': true,
        'patientDateOfBirth': this.associateUser.dateOfBirth,
        'patientContactNo': this.associateUser.contactNo
      });
      this.isDob = true;
      this.isExistingMinorPatient = true;
      this.isGender = true;
      this.getUserAddresBtRefNoAndAddressType(userRefNo,"Inpatient"); //app#2367
      //app#2367
      // this.user_refNo = userRefNo;
  ...
  onDateChange(event) {
    ...
    // let dateSelected = date.getDay() + '/' + date.getMonth() + '/' + date.getFullYear();
    // var CurrentDate = new Date();
    // let dateCurrent = CurrentDate.getDay() + '/' + CurrentDate.getMonth() + '/' + CurrentDate.getFullYear();
    // let from = new Date(dateSelected);
    // let to = new Date(dateCurrent);
    let dateCurrent = parseInt(this.convertDateToNo(new Date()));
    let dateSelected = parseInt(this.convertDateToNo(event));
    if(dateSelected < dateCurrent){
      this._toastService.showI18nToast('You are creating a back dated appointment','warning');
      // alert('Given date is past from the current date');
    }
...
//app#2367
  ...
  editModeOn(editFor) {
    if(editFor == 'name') {
      this.editFor.name = true; this.editFor.dateOfBirth = false; this.editFor.gender = false;
      this.appointmentForm.patchValue({
        'patientDateOfBirth': this.initialFormValue.patientDateOfBirth,
        'patientGender': this.initialFormValue.patientGender
      });
    } else if(editFor == 'dob') {
      this.editFor.name = false; this.editFor.dateOfBirth = true; this.editFor.gender = false;
      this.appointmentForm.patchValue({
        'patientName': this.initialFormValue.patientName,
        'patientGender': this.initialFormValue.patientGender
      });
    } else if(editFor == 'gender') {
      this.isGender = false;
      this.editFor.name = false; this.editFor.dateOfBirth = false; this.editFor.gender = true;
      this.appointmentForm.patchValue({
        'patientName': this.initialFormValue.patientName,
        'patientDateOfBirth': this.initialFormValue.patientDateOfBirth
      });
      ...
      this.editFor.name = false; this.editFor.dateOfBirth = false; this.editFor.gender = false;
    }
  }
  ...
  saveIndividualDetails() {
    ...
    let query = {
      "groupId": null,
      "userRefNo": formValue.userRefNo,
      "firstName": formValue.patientName,
      "gender": formValue.patientGender,
      "contactNo": formValue.patientContactNo,
      "emailAddress": formValue.patientEmailId,
      "dateOfBirth": formValue.patientDateOfBirth,
    }
    this._individualService.updateUserProfile(query).subscribe(resp => {
      if(resp.status == 2000) {
        this._doctorService.getAppointmentsViewByRefNo({appointmentRef: formValue.appointmentRefNo}).subscribe(response => {
          if(response.status == 2000) {
            this.appointmentForm.patchValue({
              'patientName': response.data.patientName,
              'patientDateOfBirth': response.data.userDob,
    ...
    onChamberChange(){
      this.checkHolidayByAppointmentDateAndChamber();
    }
    ...
    updateAppointment(){
      ...
      console.log(this.appointmentForm.value);
      let query: any = this.appointmentForm.value;
