...
@Component({
  selector: 'app-sample-collected',
  templateUrl: './sample-collected.component.html',
  styleUrls: ['./sample-collected.component.css']
})
export class SampleCollectedComponent implements OnInit {
  ...
  saveDeliveryStep() {
    ...
    if (this.labDeliveryForm.valid) {
      let deliveryEventDate = this.labDeliveryForm.value.collectionDate;
      let deliveryEventTime = this.labDeliveryForm.value.collectionTime;
      ...
      payload = {
        ...
        deliveryStatus: SBISConstants.LAB_DELIVERY_CONST.SAMPLE_COLLECTED,
        // expectedDelivery: expDeliveryDt[2]+"-"+expDeliveryDt[1]+"-"+expDeliveryDt[0],
        //deliveryEventDateTime: this.labDeliveryForm.value.collectionDate,
